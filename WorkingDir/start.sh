...
# ============================================
# Detect original user for audio/display
# ============================================

# Get the user who ran sudo (for audio/display access)
if [ -n "$SUDO_USER" ]; then
    REAL_USER="$SUDO_USER"
    REAL_UID="$SUDO_UID"
    log_debug "Detected user: $REAL_USER (UID: $REAL_UID)"
else
    # Fallback: Not run with sudo (direct root login or system service)
    REAL_USER=$(who | awk '{print $1}' | head -n1)
    if [ -z "$REAL_USER" ]; then
        # No one logged in, try to find user with active X session
        REAL_USER=$(ps aux | grep -E 'Xorg|X11' | grep -v grep | awk '{print $1}' | head -n1)
    fi
    REAL_UID=$(id -u "$REAL_USER" 2>/dev/null || echo "1000")
    log_warning "SUDO_USER not set, detected: $REAL_USER (UID: $REAL_UID)"
fi
...
# Set XDG runtime directory (required for PulseAudio/PipeWire)
export XDG_RUNTIME_DIR="/run/user/$REAL_UID"

# Set PulseAudio runtime path and cookie
...
log_info "Audio environment configured for user: $REAL_USER"
log_debug "  XDG_RUNTIME_DIR: $XDG_RUNTIME_DIR"
log_debug "  PULSE_COOKIE: $PULSE_COOKIE"

# Validate paths (warnings only, don't fail)
if [ ! -d "$XDG_RUNTIME_DIR" ]; then
    log_warning "Runtime directory not found: $XDG_RUNTIME_DIR"
    log_warning "User $REAL_USER may not be logged in"
fi

if [ ! -f "$PULSE_COOKIE" ]; then
    log_warning "PulseAudio cookie not found: $PULSE_COOKIE"
...
# ============================================
# CLEANUP: Stop any existing instances
# ============================================
# This allows start.sh to be run multiple times safely (idempotent restart)

echo -e "\n${CYAN}=== Cleaning Up Previous Instances ===${NC}\n"

# 1. Stop Python web server
echo -e "${YELLOW}[Cleanup 1/5]${NC} Stopping any existing web server..."
if [ -f /tmp/portal-server.pid ]; then
    PID=$(cat /tmp/portal-server.pid)
    if kill -0 $PID 2>/dev/null; then
        kill $PID 2>/dev/null || true
        log_debug "  Stopped web server (PID: $PID)"
    fi
    rm -f /tmp/portal-server.pid
fi
# Also kill any remaining server.py processes (even if PID file is missing/stale)
pkill -f "python3 server.py" 2>/dev/null || true
pkill -f "server.py" 2>/dev/null || true
log_success "Web server cleanup complete"

# 2. Stop dnsmasq
echo -e "${YELLOW}[Cleanup 2/5]${NC} Stopping any existing dnsmasq..."
if pgrep -x "dnsmasq" > /dev/null; then
    killall dnsmasq 2>/dev/null || true
    log_debug "  Stopped dnsmasq"
fi
log_success "dnsmasq cleanup complete"

# 3. Stop hostapd
echo -e "${YELLOW}[Cleanup 3/5]${NC} Stopping any existing hostapd..."
if pgrep -x "hostapd" > /dev/null; then
    killall hostapd 2>/dev/null || true
    log_debug "  Stopped hostapd"
fi
log_success "hostapd cleanup complete"

# 4. Clear iptables rules (full flush for clean slate)
echo -e "${YELLOW}[Cleanup 4/5]${NC} Clearing iptables rules..."
...
# 5. Reset interface
echo -e "${YELLOW}[Cleanup 5/5]${NC} Resetting interface $INTERFACE..."
log_debug "  Flushing IP addresses"
ip addr flush dev $INTERFACE 2>/dev/null || true
log_debug "  Bringing interface down"
ip link set $INTERFACE down 2>/dev/null || true
log_debug "  Restoring NetworkManager management temporarily"
nmcli device set $INTERFACE managed yes 2>/dev/null || true
sleep 1
log_success "Interface reset complete"
